...
class MySQLClient(object):
    ...
    def __init__(self, conf):
        self.host = conf.get("MYSQL_HOST", "127.0.0.1")
        self.port = conf.get("MYSQL_PORT", 3306)
        self.user = conf.get("MYSQL_USER", "")
        self.password = conf.get("MYSQL_PASSWORD", "")
        self.db = conf.get("MYSQL_DB", "")
        self.charset = conf.get("MYSQL_CHARSET", "utf8")
...
if __name__ == "__main__":
    conf = dict(MYSQL_HOST="192.168.190.147",
        MYSQL_PORT = 3306,
        MYSQL_USER="myblog",
    ...
    print(mysql_client.fetch_all(sql))
